...
public class CoordinatesUtil {
  ...
  private static final BigDecimal DEGREES_IN_LATITUDE_METER = new BigDecimal(
      1.0d / 110_574d).setScale(COORDINATES_ACCURACY_DECIMALS,
          RoundingMode.HALF_EVEN);
  // 1 longitude degree on equator is approximately 111.320 km
  private static final BigDecimal METERS_IN_LONGITUDE_DEGREE = new BigDecimal(
      111_320).setScale(COORDINATES_ACCURACY_DECIMALS);
  ...
  public static BigDecimal addLat(Coordinates center, int meters) {
    return DEGREES_IN_LATITUDE_METER.multiply(BigDecimal.valueOf(meters))
        .add(center.getLat());
  }
  ...
  public static BigDecimal addLng(Coordinates center, int meters) {
    ...
    // because the earth is flat, you'll get less meters per degree when you get further from the equator
    BigDecimal latitudeMultiplier = BigDecimal.valueOf(
        Math.cos(latitudeInRadians));
    return
    new BigDecimal(meters).divide(METERS_IN_LONGITUDE_DEGREE.multiply(
        latitudeMultiplier),
        COORDINATES_ACCURACY_DECIMALS, RoundingMode.HALF_EVEN)
        .add(center.getLng());
  }
